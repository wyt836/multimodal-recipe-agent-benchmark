...
\usepackage{float}             % keep key report figures/tables inside their sections
\usepackage{placeins}          % provide \FloatBarrier for float boundaries
\usepackage{tikz}
...
\email{s4779072@student.uq.edu.au}
...
\maketitle
\pagestyle{plain}        % suppress acmart's running author/title header
\thispagestyle{plain}    % also apply to this (first) page
...
Figure~\ref{fig:ablation} isolates each V3 ablation against the full
agent, making the planner regression visible by query family.
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{eval/results/figures/failure_taxonomy.png}
\caption{Failure taxonomy across systems. The stacked bars show how
failures shift from missing retrieval in V0 toward reasoning, grounding,
and citation errors once retrieval and agent components are introduced.}
\label{fig:failure-taxonomy}
\end{figure}

Table~\ref{tab:failures} and Figure~\ref{fig:failure-taxonomy} show the
5-class failure distribution. Three patterns stand out: (i)~V0 is
dominated by \textit{retrieval} failures (10/14) --- it cannot retrieve,
so it hallucinates or defers; (ii)~V1 and V2 shift toward
\textit{reasoning} failures, indicating that retrieval works but
multi-hop aggregation breaks down (e.g. V1 on QF3.1 mentions only the
winner without listing alternatives, failing
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{eval/results/figures/cost_vs_quality.png}
\caption{Cost--quality trade-off across configurations. Systems closer
to the upper-left achieve higher task success with lower token cost;
V3-no-planner improves quality but is the most expensive configuration.}
\label{fig:cost-quality}
\end{figure}

Table~\ref{tab:efficiency} and Figure~\ref{fig:cost-quality} show that
